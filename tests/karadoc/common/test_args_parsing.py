...
class ArgsParsing(unittest.TestCase):
    def test_sub_commands(self):
        ...
        args = parser.parse_args(["run", "--dry", "--tables", "schema.table1", "schema.table2"])
        self.assertEqual(args.command_0, "run")
        self.assertEqual(args.tables, ["schema.table1", "schema.table2"])

    def test_variable_parsing(self):
        parser = ArgumentParser()
        add_commands_to_parser(parser, {"run": RunCommand}, command_depth=0)
        args = parser.parse_args(["run", "--vars", "k1=v1", "k2=v2", "--tables", "schema.table1", "schema.table2"])
        self.assertEqual(args.command_0, "run")
